...
export function generateConfig(rootDir: string, stack: DetectedStack): string {
  ...
  return `// aidoc.config.ts -- generated by npx aidoc-kit init
// Edit this file to customize aidoc-kit behavior.
...
${agentsBlock}  ignore: [
  ...
  enrich: {
    ...
    // aidoc-kit resolves the model automatically via the provider API.
    // Uncomment and set "model" only if resolution fails
    // (network unavailable, invalid key, /models API not supported).
    //
    // Available models: ${docsUrl}
    //
    // @ai-maintainer: if enrich fails with "Model not resolved", fill in this field.
    // model: '${defaultModel}',
  },
}
`
...
export function runInit(rootDir: string): void {
  ...
  if (!detected) console.log('ℹ️  No known dependencies detected — minimal config generated.')

  // Refuse to overwrite an existing config
  if (existsSync(configPath)) {
    console.log('\n⚠️  aidoc.config.ts already exists — not overwritten.')
    console.log('   Delete it manually and re-run init to regenerate.\n')
    return
  }
  ...
  console.log('\n📄 aidoc.config.ts generated at project root')
  ...
  if (existsSync(gitignorePath)) {
    ...
    if (!gitignore.includes('.codemod')) {
      ...
      console.log('📝 .codemod/ added to .gitignore')
      console.log('   (remove this line if you want to commit the AI context)')
    }
  }

  console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
  console.log('✅ aidoc-kit initialized successfully!')
  console.log('')
  console.log('Next steps:')
  console.log('  1. npx aidoc-kit scan --dry     ← preview generated blocks')
  console.log('  2. npx aidoc-kit scan --write   ← write @ai-* blocks to every file')
  console.log('  3. npx aidoc-kit chunk          ← summarize large files for agents')
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n')
}
